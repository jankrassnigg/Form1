...
class OneDriveMusicSource(private val cache: OneDriveCacheManager) : MusicSource {

    override val name = "OneDrive"

    override suspend fun listFolder(folderId: String?): Result<FolderContents> {
        return cache.listFolderContents(folderId).map { contents ->
            FolderContents(
